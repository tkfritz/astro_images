...
rot_image_train,rot_target_train,rot_df_train=get_noise_all(image_train[:,:,:,:],target_train[:],df_train.iloc[:,0:51],nmult=8,noise_level=0.761*6)
stop_time=time.time()
...
def fit_model(model=0,modeltype="perceptron",l2reg=0.01,end_epochs=200,keep_prob=1,start_epochs=15,start_reg=0.3,max_depth=6,alpha=0.001,num_features=1849,rot_image_train=rot_image_train,rot_target_train=rot_target_train,rot_df_train=rot_df_train):
    ...
    if modeltype=="perceptron":
        ...
        rot_df_train = np.array(rot_df_train.iloc[:,51:1900])
        ...
        PATH='/home/tobias/ml-testing/astr-images/mlp_4layers_reg'+str(best_reg)+'_noi_'+str(end_epochs)+'epochs.pkl'
        ...
        PATH='/home/tobias/ml-testing/astr-images/conv2d_2layers_reg'+str(best_reg)+'_noi_'+str(end_epochs)+'epochs.pkl'
        torch.save(model3.state_dict(), PATH)
        stop_time=time.time()
